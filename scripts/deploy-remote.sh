...
SSH_TARGET=""
SSH_LOGIN_USER=""
SSH_CONTROL_PATH="${TMPDIR:-/tmp}/nsmp-ssh-control"
...
render_header() {
	...
	printf '\n'
	hr
	panel_line "$(status_info "nsmp deploy installer")"
	hr
	...
	panel_line "build    : $([[ "$SKIP_BUILD" == true ]] && printf 'skip local build' || printf 'npm ci + npm run build (root + ui)')"
...
Options:
  ...
  --install-dir <path>     Remote install path (default: /var/www/nsmp)
...
Environment overrides:
  NSMP_SSH_TARGET, NSMP_PORT, NSMP_INSTALL_DIR,
  NSMP_SERVICE_USER, NSMP_SERVICE_GROUP, NSMP_PROXY,
  NSMP_DOMAIN, NSMP_CADDY_EMAIL, NSMP_SSH_PORT,
  NSMP_DRY_RUN, NSMP_SKIP_BUILD
EOF
}
...
build_app() {
	...
	step_begin "install root dependencies"
	(
		cd "$REPO_ROOT"
		npm ci
	)
	...
	step_begin "build root (TypeScript)"
	(
		cd "$REPO_ROOT"
		npm run build
	)
	...
	step_begin "install UI dependencies"
	(
		cd "$REPO_ROOT/ui"
		npm ci
	)
...
managed_marker="# managed by nsmp"
...
printf '\n'
hr
panel_line "nsmp remote installer"
hr
panel_line "install  : ${INSTALL_DIR}"
panel_line "service  : ${SERVICE_NAME}"
panel_line "port     : ${PORT}"
...
User=${SERVICE_USER}
Group=${SERVICE_GROUP}
ExecStart=python3 ${INSTALL_DIR}/scripts/spa-http-server.py ${INSTALL_DIR}/ui/dist --bind 127.0.0.1 --port ${PORT}
Restart=always
RestartSec=5
...
detect_caddy_snippet_dir() {
	...
	local dir
	if [[ -n "${NSMP_CADDY_SNIPPET_DIR:-}" ]]; then
		printf '%s' "$NSMP_CADDY_SNIPPET_DIR"
		return 0
	fi
	if [[ -f "$main_file" ]]; then
...
${import_line}"
		...
		if grep -qE '^[[:space:]]*import[[:space:]].*(/etc/caddy/)?(conf\.d|Caddyfile\.d)/\*\.caddy' "$main_file"; then
			...
			log "existing Caddyfile is managed by nsmp; adding snippet import"
...
detect_nginx_style() {
	...
	if [[ -n "${NSMP_NGINX_STYLE:-}" ]]; then
		printf '%s' "$NSMP_NGINX_STYLE"
		return 0
	fi
...
if [[ "$PORT" == "3000" && -n "${NSMP_PORT:-}" ]]; then
	PORT="$NSMP_PORT"
fi
if [[ "$INSTALL_DIR" == "/var/www/nsmp" && -n "${NSMP_INSTALL_DIR:-}" ]]; then
	INSTALL_DIR="$NSMP_INSTALL_DIR"
fi
if [[ "$SERVICE_USER" == "www-data" && -n "${NSMP_SERVICE_USER:-}" ]]; then
	SERVICE_USER="$NSMP_SERVICE_USER"
fi
if [[ "$SERVICE_GROUP" == "www-data" && -n "${NSMP_SERVICE_GROUP:-}" ]]; then
	SERVICE_GROUP="$NSMP_SERVICE_GROUP"
fi
if [[ "$PROXY_MODE" == "auto" && -n "${NSMP_PROXY:-}" ]]; then
	PROXY_MODE="$NSMP_PROXY"
fi
if [[ -z "$DOMAIN" ]]; then
	DOMAIN="${NSMP_DOMAIN:-}"
fi
if [[ -z "$CADDY_EMAIL" ]]; then
	CADDY_EMAIL="${NSMP_CADDY_EMAIL:-}"
fi
if [[ "$SSH_PORT" == "22" && -n "${NSMP_SSH_PORT:-}" ]]; then
	SSH_PORT="$NSMP_SSH_PORT"
fi
if [[ "$DRY_RUN" == false && -n "${NSMP_DRY_RUN:-}" ]]; then
	is_true "$NSMP_DRY_RUN" && DRY_RUN=true || true
fi
if [[ "$SKIP_BUILD" == false && -n "${NSMP_SKIP_BUILD:-}" ]]; then
	is_true "$NSMP_SKIP_BUILD" && SKIP_BUILD=true || true
fi
